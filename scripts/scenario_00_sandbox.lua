...
function init()
  ...
  ds197 = SpaceStation():setTemplate("Small Station"):setFaction("Independent"):setCallSign("DS197"):setPosition(43522, -33586)

  planwoah = Planet():setPosition(60000, 60000):setPlanetRadius(5000):setPlanetSurfaceTexture("planets/planet-1.png"):setPlanetCloudTexture("planets/clouds-1.png"):setPlanetAtmosphereTexture("planets/atmosphere.png"):setPlanetAtmosphereColor(0.2, 0.2, 1.0)
  misswoah = HomingMissile():setPosition(50000, 50000):setTarget(ds197):setLifetime(600)
  planwoah:setOrbit(misswoah, 2)

end

function update(delta)

  -- Drain energy at a rate of 0.1%/s (calculated per-tick)
  -- --
  -- Instead of being able to run every system at 100% indefinitely, it takes
  -- a reactor running at 240% to support 100% in all other systems and maintain
  -- stable energy stores. At this setting, the reactor will slowly take on heat
  -- even with 100% coolant in it. The means you MUST run some systems under
  -- 100% in order to be energy positive. 
  -- Season to taste.
  -- -- 
  local energy_drain = player:getEnergyLevelMax() * 0.001 * delta
  player:setEnergyLevel(player:getEnergyLevel() - energy_drain)
